...
\begin{itemize}
\item[$\ket*{\varphi_{i}}$]
  Laguerre basis state

\item[$\hat{A}$]
  anti-symmetriser operator
\item[$\hat{P}_{i, j}$]
  pairwise exchange operator

\item[$\hat{H}_{T}$]
  target Hamiltonian operator
\item[$\hat{K}_{m}$]
  target electron kinetic operator
\item[$\hat{V}_{m}$]
  target electron potential operator
\item[$\hat{V}_{m, n}$]
  target electron-electron potential operator
\item[$\hat{H}_{T, e}$]
  target Hamiltonian operator, restricted to one target electron

\item[$\ket*{\phi_{i}}$]
  one-electron atomic orbital
\item[$\ket*{\chi_{i}}$]
...
\item[$\ket*{\Phi_{n}^{\lr{N}}}$ / $\epsilon_{n}^{\lr{N}}$]
  target pseudostate / pseudoenergy calculated with $N$ one-electron basis
  states
\item[$\ket*{\Phi_{n}^{\lr{C, N}}}$ / $\epsilon_{n}^{\lr{C, N}}$]
  target pseudostate / pseudoenergy calculated with $C$ core states, and $N$
  one-electron basis states

\item[$\hat{H}$]
  total Hamiltonian operator
\item[$\hat{H}_{A}$]
  asymptotic Hamiltonian operator
\item[$\hat{V}$]
  potential operator
\item[$E$]
  total energy
\item[$\ket*{\Psi}$]
  total wavefunction
\item[$\ket*{\Omega_{\alpha}}$]
  asymptotic eigenstates
\item[$\ket*{\Omega_{\alpha}^{\lr{E}}}$]
  asymptotic on-shell eigenstates
\item[$\hat{G}_{\lr{E}}$]
  Green's operator
\end{itemize}
