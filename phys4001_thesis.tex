...
With the aim of calculating convergent TIECS for helium using the CCC method,
we first establish how convergence is to be obtained.
The parameters which most uniquely define our CCC calculations are:
\begin{itemize}
\item[$N$]
  the number of one-electron basis states used in the construction of the
...
\item[$C$]
  ...
  that may be used for the core orbitals in the Slater determinants used to
...
With regard to the issue of finding convergence in a multi-parameter setting, we
observed that, for fixed values of $N$ and $\lambda$, the calculations
$\mathrm{CCC}\lr{C, N, \lambda}$ converged with regard to $C = 1, 2, \dotsc, N$.
We see justification for this with the fact that the parameters $N$ and
$\lambda$ define the one-electron atomic orbitals, with the nature of these
orbitals being independent of the parameter $C$.
These one-electron atomic orbitals are then used in anti-symmetrised
combinations to construct the Slater determinants and thus the target
pseudostates, with the number of pseudostates depending on $C$.
Qualitively, the parameters $N$ and $\lambda$ define the overall scope of the
space of target states, while the parameter $C$ tends to fill in the finer
details.

Hence, we sought to first obtain, for fixed values of the parameters $N$ and
$\lambda$, calculations which had converged with regard to the parameter $C$, in
the sense of
\begin{equation*}
...
parameters on the TIECS calculations proved to be more nuanced.
...
This is balanced by the fact that the TIECS for these calculations tended to
...
\begin{figure}[h]
  \begin{center}
    % \input{figures/cs_tics_iw/2_35/figure.tex}
  \end{center}
  \caption[...]{...}
  % \label{fig:}
\end{figure}
...
\begin{figure}[h]
  \begin{center}
    % \input{figures/cs/2_35/figure.tex}
  \end{center}
  \caption[...]{...}
  % \label{fig:}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption[Singlet Helium Pseudoenergies - Auto-Ionising Region]{
    ...
    A number of pseudoenergies are annotated by their corresponding major
    configuration across ranges of $\lambda$ values, including discrete
